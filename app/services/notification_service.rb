class NotificationService
  ...
  def self.contractor_invoice_ineligible_event(
    application,
    primary_contact,
    employee_contact
  )
    notification_user_hash = {}
    notification_data = application.publish_invoice_ineligible__data
    recipients = [primary_contact, employee_contact].compact.uniq(&:id)

    recipients.each do |recipient|
      notification_user_hash[recipient.id] = notification_data
      PermitHubMailer.contractor_invoice_ineligible(
        application,
        recipient
      )&.deliver_later
    end
  ...
  def self.contractor_invoice_draft_reminder_event(
    application,
    primary_contact,
    employee_contact
  )
    notification_user_hash = {}
    notification_data = application.publish_invoice_draft_reminder__data
    recipients = [primary_contact, employee_contact].compact.uniq(&:id)

    recipients.each do |recipient|
      notification_user_hash[recipient.id] = notification_data
      PermitHubMailer.contractor_invoice_draft_reminder(
        application,
        recipient
      )&.deliver_later
    end
