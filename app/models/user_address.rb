class UserAddress < ApplicationRecord
  ...
  enum :address_type, { physical: "physical", mailing: "mailing" }, prefix: true
  ...
  def only_one_address_per_type
    if address_type_physical? && user.physical_address &&
         user.physical_address.id != id
      errors.add(:base, :one_physical)
    end

    if address_type_mailing? && user.mailing_address &&
         user.mailing_address.id != id
  ...
  def enforce_unique_address_type
    if address_type_physical?
      if user.physical_address && user.physical_address.id != id
        user.physical_address&.destroy
      end
    elsif address_type_mailing?
      if user.mailing_address && user.mailing_address.id != id
