...
export const ExternalApiKeyModalSubRoute = observer(function ExternalApiKeyModalSubRoute() {
  ...
  const externalApiKey = currentProgram?.getExternalApiKey(externalApiKeyId);
  ...
  const onClose = () => {
    navigate(`/programs/${currentProgram?.id}/api-settings`);
  };

  useEffect(() => {
    reset(formFormDefaultValues(externalApiKey));

    if (currentProgram && externalApiKey) {
      (async () => {
        const apiKey = await currentProgram.fetchExternalApiKey(externalApiKeyId);

        apiKey?.token && setToken(apiKey.token);
      })();
    }
  }, [currentProgram, externalApiKey]);

  const onSubmit = handleSubmit(async (data) => {
    const updatedExternalApiKey = externalApiKey
      ? await currentProgram.updateExternalApiKey(externalApiKey.id, data)
      : await currentProgram.createExternalApiKey(data);

    if (updatedExternalApiKey) {
      externalApiKey
        ? onClose()
        : navigate(`/programs/${currentProgram?.id}/api-settings/${updatedExternalApiKey.id}/manage`);
    }
  });
  ...
  const onRevoke = async () => {
    ...
    const updatedExternalApiKey = await currentProgram.revokeExternalApiKey(externalApiKey.id);
                  ...
                  render={({ field: { onChange, value } }) => {
                    return (
                      <SandboxSelect
                        onChange={onChange}
                        value={value}
                        options={currentProgram.sandboxOptions}
                        includeLive
