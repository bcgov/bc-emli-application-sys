...
export const ExternalApiKeysIndexScreen = observer(function ExternalApiKeysIndexScreen({ ...containerProps }: IProps) {
  ...
  useEffect(() => {
    if (currentProgram) {
      (async () => {
        ...
        await currentProgram.fetchExternalApiKeys();
  ...
  if (!currentUser || !currentProgram) return <LoadingScreen />;

  const externalApiKeys = currentProgram.externalApiKeys;
  const externalApiEnabled = currentProgram.externalApiEnabled;
  ...
  return (
    <Container maxW="container.lg" p={8} as={'main'} h={'full'} w={'full'} {...containerProps}>
      ...
      <VStack alignItems={'flex-start'} spacing={5} w={'full'} h={'full'}>
        ...
        {currentProgram?.externalApiState === EJurisdictionExternalApiState.gOff && currentUser.isManager && (
          <CalloutBanner
            type={'warning'}
            title={
              <>
