    ...
    <Box display={'contents'} role={'rowgroup'}>
      <Box display={'contents'} role={'row'}>
        ...
        >
          ...
          {currentUser.isManager && currentUser.program.externalApiState === EJurisdictionExternalApiState.gOff ? (
...
const ExternalApiEnabledSwitchWithConfirmation = observer(() => {
  ...
  const isDisabled = currentUser.isManager && currentProgram.externalApiState === EJurisdictionExternalApiState.gOff;

  const shouldUseDisableConfirmationModal =
    !isDisabled && currentProgram.externalApiEnabled && currentProgram.externalApiKeysMap.size > 0;

  return shouldUseDisableConfirmationModal ? (
    <RemoveConfirmationModal
      renderTriggerButton={({ onClick }) => (
        <ExternalApiEnabledSwitch
          externalApiEnabled={currentProgram.externalApiEnabled}
      ...
      onRemove={currentProgram.toggleExternalApiEnabled}
    />
  ) : (
    <ExternalApiEnabledSwitch
      externalApiEnabled={currentProgram.externalApiEnabled}
      isDisabled={isDisabled}
      onChange={currentProgram.toggleExternalApiEnabled}
    />
  );
});
