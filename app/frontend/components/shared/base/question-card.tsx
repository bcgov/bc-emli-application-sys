...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, answers, onAnswerSelect, type }) => {
  ...
  // Reset selection if current selection is not in the new answers array
  useEffect(() => {
    if (selectedAnswer && !answers.includes(selectedAnswer)) {
      setSelectedAnswer('');
    }
        ...
        <CardHeader>
          ...
          <Text fontSize="md" mt="2">
            ...
            {type === t('auth.checkEligibility.assesmentText') && (
              <Link href={t('auth.checkEligibility.BCAssessmentLink')} target="_blank" rel="noopener noreferrer">
                {t('auth.checkEligibility.BCAssessment')}
                <ArrowSquareOut aria-label="(opens in new window)" />
              </Link>
            )}
          </Text>
          {additionalContent && (
