  ...
  types
    ...
    .views((self) => ({
      ...
      get formattedFormJson() {
        ...
        const ephemeralProcessedFormJson = self.isEphemeral
          ? processFieldsForEphemeral(clonedFormJson)
          : clonedFormJson;
    ...
    .views((self) => ({
      ...
      canUserManageCollaborators(user: IUser, collaborationType: ECollaborationType) {
        if (collaborationType === ECollaborationType.review) {
          return (!self.isDraft || self.isRevisionsRequested) && user.isReviewStaff;
        }
...
export interface IEnergySavingsApplication extends Instance<typeof EnergySavingsApplicationModel> {}
